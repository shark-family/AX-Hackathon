...
async def generate_pdf_endpoint(request: ResumeData, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # ▶ PDF는 생성됐는데 returncode만 0이 아닐 때: 경고만 출력하고 계속 진행
        if process.returncode != 0:
            print(f"⚠️ xelatex가 경고/에러를 냈지만 PDF는 생성되었습니다. returncode = {process.returncode}")
            log_file = temp_dir / "resume.log"
            if log_file.exists():
                with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
                    lines = f.readlines()
                    print("---- resume.log tail ----")
                    print(''.join(lines[-10:]))
                    print("--------------------------")
